...
module.exports.getSeasonCode = async ()=>{
  var now = new Date();
  //var now = Date.parse("2018-07-01T00:00:00.000Z");
  ...
  if(now >= setting.currentSeasonEndAt){
    ...
    update({$set:{
      ...
      currentSeasonStartAt: setting.currentSeasonStartAt.setMonth(setting.currentSeasonStartAt.getMonth() + setting.monthsPerSeason),
...
module.exports.getNewSeasonName = (seasonStart,monthsPerSeason)=>{
  ...
  var monthEnd = month + seasonLength - 1;
